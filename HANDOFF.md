...
공개 이벤트 수신 도메인은 `brief.b4ton.com`으로 정했다. 서버는 미구축 상태다. 양쪽
저장소의 도메인 설정 예시를 맞췄으며 DNS 연결·공인 인증서 발급·실제 배포는 하지 않았다.
다음 입력은 서버 위치·접속 방법·실제 IP다. 같은 서버에 배치하면 공개 Caddy의 80·443
포트를 통합하고, 여러 서버에 배치하면 기존 Docker 공유 네트워크를 대신할 비공개 경로를
먼저 준비해야 한다. 지표 수집 시스템·경보 수신처도 미정이다.
[도메인 배포 안내](docs/operations/brief-b4ton-com-deployment.md)에 준비 순서를 기록했다.
설정 예시로 공개·서비스·지표 Compose 구문과 호스트 반영을 확인하고 문서 링크를 검증했다.
설정 예시·문서 변경이므로 제품 테스트와 이미지 빌드는 반복하지 않았다.

BATON BRIEF의 로컬 MVP와 스테이징용 기본 실행 구성을 만들었다.

- 이벤트 v1·v2 멱등 수신, 최초 충돌 증거, 집계 리비전과 공백 처리
- 현재 관심 항목 단건·상태별 필터 목록·활성 요약·상태 전이 증거와 원자적 전체 재구축
...
BRIEF의 별도 서비스 Bearer와 비공개 HTTPS 앞단, BATON 조회 client와 V27 생성 실행 기록을
...
Unicode 경계 입력의 생산자 검증으로 확대하지 않는다. 당시 BATON 저장소의 계약 고정 버전은
`2.0.0-rc.1`이었으며 임시 복사본의 성공은 버전 고정 변경을 뜻하지 않았다. 양쪽 저장소
...
2026-08-31 BATON `codex/brief-attention-view`에서 `2.0.0-rc.4`의 VERSION·Schema·일곱 예시를
실제로 고정하고 기존 record 직렬화 테스트를 같은 입력으로 전환했다. 이전 `rc.1`은 보존했다.
고정한 9개 파일은 BRIEF 원본과 같으며 계약 ZIP SHA-256은 BATON `contracts/brief/README.md`에
기록했다. 기존 `BriefDeliveryEndToEndTest`도 실제 두 JAR과 MySQL 8.4·PostgreSQL 18.6에서
성공했다. 이벤트 모델·outbox·송신기와 BRIEF 소비자 구현은 바꾸지 않았으며 공인 HTTPS 검증은
여전히 남아 있다.

## 현재 검증 근거

### 2026-09-05 두 저장소 메인 병합

BRIEF의 에디션 분류·조회와 문구 개선을 로컬 `main`에 반영했다. BATON도 `1916d8c8`에서
메인의 계정 권한·분리된 화면 구조와 BRIEF 조회·생성·탐색 기능을 병합했다. 계정 권한 팀은
공유 키 없이 조회할 수 있고 열람자의 생성 버튼은 비활성화한다. 다른 작업 폴더는 유지했다.

BRIEF `test :bootstrap:bootJar`는 기존 결과를 재사용해 성공했다. BATON `build checkApiContract`,
후속 REST Docs 수정의 `generateApiContract checkApiContract`와 최종 프런트 빌드도 성공했다.
전체 API 대역 브라우저 검증은 614건이 통과하고 기존 실행 조건에 따라 43건을 제외했다.
열람자 제한 보완 후 관련 시나리오 54건도 통과했다. BATON 문서의 상대 경로 128개를 확인했다.
원격 저장소 push, 실제 DB를 사용하는 브라우저 통합·교차 서비스·공인 스테이징 검증은
이번 병합에서 실행하지 않았다.

### 2026-09-05 화면·오류·문서 문구 정리

BATON 화면을 ‘BRIEF 업무 점검’, ‘미해결·해결’, ‘원본 변경 번호’, ‘기록 누락 이력’으로
맞췄다. 오늘 화면·로그인·검색·저장 안내와 BRIEF 오류 메시지, 양쪽 README·PRD도 정리했다.
문제 해결과 업무 완료의 구분, API 필드·상태 코드와 판정 기준은 유지했다.

BATON 프런트 빌드, `BriefApplicationServiceTest`와 `BriefEditionSecurityTest`가 통과했다.
브라우저는 최종 BRIEF 39건과 관련 공통 화면 28건이 통과했다. 모바일 전용 1개 시나리오는
데스크톱 프로젝트 두 곳에서 제외했다. 모바일 화면과 인쇄물의 문구 배치도 확인했다.
문구 변경이므로 전체 백엔드·교차 서비스·실제 스테이징 검증은 반복하지 않았다.

추가 검토에서 BATON 역할·운영·결정·인수인계 화면 제목, 구성원 활동 재개와 자료 설명을
정리했다. 루틴 완료를 인수인계 전달로 안내하지 않으며, 인증 화면은 로그인·계정 만들기·
이메일 인증을 구분한다. 이메일 인증 완료는 서버 성공 뒤에만 안내하고 위험 항목의 미완료
건수에는 전달 당시 기준임을 명시한다. 프런트 빌드, `ContinuitySignalAnalyzerTest`와 관련
API 대역 브라우저 67건이 통과했다. 모바일 전용 시나리오의 데스크톱 두 건은 제외했으며,
전체 백엔드·실제 DB를 사용하는 브라우저 통합·스테이징 검증은 반복하지 않았다.

후속 문구 검토의 12개 항목도 반영했다. BATON의 체크리스트 완료율·항목 수와 전달 전
확인사항, 보관 여부 필터, 루틴 기한 입력과 항목 완료 안내를 정리했다. 요청 복구 화면은
브라우저의 임시 요청 기록 삭제와 접근 키 변경 결과 확인을 구체적으로 안내한다. 계산·상태
판정·요청 복구 처리는 유지했다. BATON 프런트 빌드와 관련 API 대역 브라우저 64건이 통과했고,
브라우저·화면 크기별 기존 제외 조건에 따라 7건은 건너뛰었다. 변경한 BATON 문서의 상대 링크
64개와 차이의 공백 오류를 확인했으며, 임시 3179 포트 설정과 화면 서버를 정리했다.
양쪽 백엔드 전체 테스트·실제 DB 브라우저 통합·스테이징 검증은 반복하지 않았다.
...
대상에서 전체 건수와 페이지를 읽으므로 다음 페이지나 빈 끝 페이지에서도 전체 건수를
유지한다. 반복 해소 갱신은 원래 해소 시점·리비전을 바꾸지 않는다. 재활성화 뒤 재구축해도
같은 조건의 상세 결과를 반환한다. 기존 경로·V9 스키마·이벤트 계약·규칙 버전은 유지했다.

`test :bootstrap:bootJar`가 성공했다. 기존 PostgreSQL 주간 해소 시나리오에 상세 값,
다음 페이지·빈 페이지, 재활성화 후 재구축과 불완전 커서 거부를 추가했다. 현재 관심 목록의
커서 입력 검증을 재사용하며 기존 필터 조회도 전체 테스트에서 확인했다.
...
### 2026-09-05 이번 주 해소 요약과 지난주 비교

[PRD-0030](docs/PRD/0030_weekly-resolution-summary/spec.md)에 따라 주간 해소 개수 조회를
추가했다. 마지막으로 적용된 활성과 바로 다음 리비전의 해소를 확인하고 현재도 해소된 항목만
센다. 반복 해소 갱신은 날짜를 옮기지 않으며 재활성화·해소 시점의 기록 공백은 제외한다.
BATON은 시즌 시간대와 현재 주차를 정해 이 수치를 활성 요약과 별도로 표시한다.
지난주 비교는 선택한 불변 브리프의 주차·시간대로 기존 주간 최신 조회를 사용한다.
V9 스키마·이벤트 계약 팩·투영 및 에디션 규칙 버전은 유지했다.

BRIEF `test :bootstrap:bootJar`가 성공했다. PostgreSQL 통합 검증은 연속 전환·반복 해소·
재활성화·중복·오래된 전달·공백·재구축·다른 범위 격리와 서울·DST 주간 경계, 고정 시각을
확인했다. 서비스 인증 테스트에서 이벤트 token과 서비스 token의 분리도 확인했다.

Compose 구문과 BRIEF·서비스 Caddy 이미지 빌드, Caddy 설정 검증 및 CI의 HTTPS 조립
시나리오가 성공했다. 고유 프로젝트와 임시 포트 38880·38843에서 신뢰한 로컬 HTTPS의
서비스 조회 `200`·무인증 `401`, 공개 경로의 같은 조회 `404`, 이벤트 무인증 `401`·정상
Bearer 수신 `202`, DB aggregate health, 비루트·읽기 전용·네트워크·capability와 Bearer
로그 비노출을 확인했다. 검증 컨테이너·볼륨·네트워크는 정리했다. 실제 공인 배포는 아니다.


이번 BATON 변경은 기능 `ac5f7e0`, 테스트 `1776a99`, 문서 `564edec`에 있다.

BATON `build checkApiContract`도 통과했다. 애플리케이션 404건·웹 118건·REST Docs 150건·
외부 어댑터 82건·영속성 어댑터 36건·부트스트랩 66건에 실패·건너뜀은 없다. 프런트 빌드와
Chromium·390px 모바일·WebKit의 BRIEF 브라우저 27건이 통과했고 임시 3179 포트를 정리했다.
실제 두 JAR·MySQL 8.4·PostgreSQL 18.6·서비스 Caddy의 `BriefEditionHttpsEndToEndTest`에서
해소 건수·권한, 지난주 없음·다른 범위 거부·정확한 시간대의 최대 생성 순번·규칙 버전이
다른 지난주 에디션의 조회도 확인했다. 이 테스트의 수신 기록과 과거 에디션은 격리 DB에
준비한 조회 자료이며 실제 이벤트 송신·공인 배포 완료의 근거가 아니다.

### BATON 탐색·업무 연결·저장 이후 변경 안내

BATON `codex/brief-edition-carryover-ui`의 PRD-0010에 따라 현재 업무명·역할/루틴 이동,
에디션 이력·비교·생성 준비 상태와 저장 이후 추가 전달 안내를 연결했다. 추가 전달은 같은
에디션의 성공 생성·재사용 기록과 BATON outbox로 판단하며, 근거가 없으면 UNKNOWN이다.
담당자·역할 준비·루틴·인수인계별 확인 버튼을 제공하고 원본 참조·리비전은 상세 기록에 둔다.
BRIEF 제품 코드·V9 스키마·이벤트 계약·공개 허용 목록은 유지했다.

이번 BATON 커밋은 기능 `9d35a29`, 테스트 `362f9de`, 문서 `8bd092b`다. 그 앞의 탐색 연결은
기능 `7da7f33`, 테스트 `f6c8e4f`, 문서 `2384056`에 있다.

BATON의 최종 `build checkApiContract`가 성공했다. 애플리케이션 기본 테스트 402건,
웹 118건·REST Docs 148건·외부 어댑터 81건·영속성 어댑터 36건·부트스트랩 66건에
실패·건너뜀은 없었다. 프런트 빌드와 API 대역의 Chromium·390px 모바일·WebKit BRIEF
브라우저 24건도 통과했다. 다른 작업의 기본 포트를 피해 임시 3179 포트를 사용하고 정리했다.

실제 BATON·BRIEF 두 JAR·MySQL 8.4·PostgreSQL 18.6·서비스 Caddy의
`BriefEditionHttpsEndToEndTest`도 성공했다. 기존 이력·비교·범위 거부·업무 연결과 함께
미확정 생성 근거, 추가 전달, 같은 에디션 재사용 뒤 안내 갱신을 확인했다. BRIEF 투영과
BATON 전달 완료 기록은 격리 DB에 준비했으며 실제 이벤트 송신·공인 스테이징 검증을 뜻하지
않는다. BRIEF는 문서만 바꿔 제품 테스트·JAR 빌드를 반복하지 않았고, 앞서 검증한 JAR을
교차 검증에 사용했다. 이후 이번 주 해소 요약·지난주 바로 비교 작업은 아래 기록을 따른다.
실제 배포·자동 생성·RELAY 전달은 남아 있다.
...
`HIGH`·`MEDIUM` 개수와 리비전 공백이 기록된 활성 항목 수를 조회한다. 기존 `attention_item`의
단일 SQL 집계와 Spring `DataClassRowMapper`를 사용한다. 새 테이블·인덱스·캐시·검증 도우미는
추가하지 않았으며 이벤트 계약 팩과 투영·에디션 규칙 버전도 유지했다.

[PRD-0028](docs/PRD/0028_attention-item-filters/spec.md)은 기존 목록에 선택적 `severity`와
`revisionGap` 필터를 추가한다. Spring MVC 타입 변환과 Kotlin `buildList`, 기존 JDBC
매개변수 바인딩을 사용하며, 기본 상태·정렬·커서·응답과 요약 집계는 유지한다.

2026-08-31 기존 v1·v2 수신 시나리오와 서비스 인증 대상 테스트에서 범위 분리·빈 요약,
심각도 변경·해소·공백·재구축 후 동일 결과와 무인증·이벤트 token 거부·서비스 token 허용을
확인했다. 이어 `./gradlew --no-daemon test :bootstrap:bootJar`가 성공했다. 변경 없는 도메인
테스트는 Gradle의 기존 성공 결과를 재사용했다. 이벤트 계약 팩은 바꾸지 않아 ZIP 생성은
반복하지 않았다.

요약 기능 추가 시점의 코드로 BRIEF·서비스 Caddy 이미지를 빌드하고 기존 CI의 Caddy 설정·HTTPS 조립
절차를 별도 프로젝트·이미지 태그·loopback 포트로 실행했다. Compose와 Caddy 설정, DB
aggregate health, 비루트·읽기 전용 실행, 내부 네트워크와 capability를 확인했다. 신뢰한
서비스 HTTPS에서 요약 `200`과 무인증 `401`, 공개 HTTPS의 같은 요약 경로 `404`, 이벤트
`202 APPLIED`와 health 비노출, token 로그 비노출을 확인했다. 검증용 컨테이너·볼륨·네트워크와
임시 인증서·비밀을 제거했다. 공인 HTTPS·실제 스테이징 비밀·원격 CI는 검증하지 않았다.

필터 추가 후 기존 v1·v2 수신 대상 테스트에서 선택 조건·교집합·필터된 키셋, 범위 분리와
상태·심각도 변경 및 재구축을 확인했다. 최종 `./gradlew --no-daemon test :bootstrap:bootJar`도
성공했고 변경 없는 도메인 테스트는 기존 성공 결과를 재사용했다. 이벤트 계약 팩·스키마·
인증 경로·Compose·Caddy는 바꾸지 않아 ZIP 생성·이미지 빌드·HTTPS 조립은 반복하지 않았다.

2026-08-31 BATON의 별도 `codex/brief-attention-view` 작업 브랜치에서 요약·필터 중계 API와
‘오늘’ 화면의 관심 항목 패널을 연결했다. 기존 세션·활성 멤버십·접근 키와 서비스 Bearer를
재사용하며 상태·심각도·공백 조건 변경과 새로고침은 첫 페이지부터 읽는다.

BATON의 기존 선택 실행 `BriefEditionHttpsEndToEndTest`를 확장해 실제 두 실행 JAR,
MySQL 8.4·PostgreSQL 18.6과 서비스 Caddy에서 새 조회의 허용·거부와 필터된 키셋을 확인했다.
조회 데이터는 격리된 BRIEF DB의 대표 투영으로 준비했으며 이벤트 생산·전달 완료를 검증한
것은 아니다. 브라우저의 요약 선택·필터·커서 초기화·빈 결과·장애·권한 거부는 API 대역을
...
실제 HTTPS 교차 테스트는 격리된 수신 기록으로 전이의 특수문자 참조·배타 과거 커서와 공백 근거를
확인했다. 브라우저 API 대역 검증은 상태 전이·과거 페이지, 미생성·전달 대기·생성·재사용,
저장 시간대·이전 근거 `null`, 종료 시즌 생성 차단과 권한 거부를 확인했다. 화면에서는 원본
판정이나 생성 실행을 복제하지 않으며, BRIEF에 새 테이블·공개 경로를 추가하지 않았다.
BATON 전체 빌드·API 계약 검사와 프런트 빌드는 통과했다. 전체 브라우저 스모크는 66건 중
63건이 처음 통과했고, 로딩 시간 초과가 난 기존 3건도 시간 제한 변경 없이 분리 재실행해 통과했다.
...
스테이징 비밀 파일의 준비 조건은 [README](README.md#스테이징-실행)에 명시했다.
2026-08-31 로컬 BRIEF 이미지의 네트워크 없는 임시 컨테이너에서 빈 파일로 권한을
확인했다. UID `10001`은 `root:root 0600` 파일을 읽지 못했고 `10001:10001 0400` 파일은
읽을 수 있었다. 실제 비밀·호스트 파일은 사용하지 않았고 임시 컨테이너는 제거했다.
이는 컨테이너 내부 파일 권한 확인이며 실제 스테이징의 secret 마운트·애플리케이션 기동이나
rootless·사용자 네임스페이스 재매핑 환경 검증은 아니다. 이번 변경은 문서·예시 주석뿐이므로
제품 빌드·테스트와 전체 Compose 조립은 반복하지 않았다.
...
현재 활성 요약도 서비스 HTTPS의 정상 개수·무인증 `401`과 공개 HTTPS의 `404`를 확인한다.
...
제한한다. 검증 뒤 임시 컨테이너·네트워크·볼륨과 비밀 파일 정리를 시도한다. 검증이
성공했어도 정리에 실패하면 CI가 실패하며, 앞선 검증 오류가 있으면 그 종료 코드를 우선한다.
...
정리 함수는 Docker·파일 삭제 명령을 대체해 정상 종료 `0`, 네트워크 삭제 실패 `37`,
앞선 검증 오류 `28`의 우선 보존을 확인했다. 네트워크 오류 출력과 후속 파일 정리 실행,
워크플로 YAML·Bash 구문도 확인했다. 실제 자원 조작·제품 빌드·전체 테스트·원격 CI는
실행하지 않았다.
...
- BATON 연결 작업 브랜치 병합과 실제 스테이징 브라우저의 요약·필터·전이·에디션 생성 확인
...
서버를 마련하고 `brief.b4ton.com`의 공개 앞단과 내부 조회·생성 네트워크를 준비한다.
기본 Compose 두 개를 같은 호스트의 기본 80·443 포트로 동시에 실행하지 않는다.
...
요약·필터·전이를 포함한 사용자 조회와 생성 연결의 로컬 구현·교차 서비스 검증은 완료했다.
다음 운영 진입점은 실제 스테이징 배포의 서비스 인증서·truststore·서로 다른 현재 token을 주입해 조회와 생성을
...
PRD-0027 요약과 PRD-0028 필터 목록, PRD-0016 전이와 최신 에디션의 BATON 중계·화면 연결은
별도 작업 브랜치에 구현했다.
다음 제품 연결 단계는 해당 브랜치를 검토·병합하고 실제 비공개 서비스 HTTPS를 활성화한
스테이징의 로그인 사용자 화면을 확인하는 것이다. BRIEF의 빈 요약은 권한 판정이 아니며
요약과 목록 사이의 요청 시점 차이와 필터 변경 시 커서 초기화 계약을 유지한다.
