...
BATON의 요약·필터 중계 API·client·화면과 교차 서비스 검증은 포함하지 않았다.
기존 조회·생성 교차 검증을 새 요약·필터의 완료 근거로 확대하지 않는다.
...
- BATON의 현재 관심 항목 요약·필터 중계 API·client·화면과 교차 서비스 검증
...
다음 제품 연결 작업은 BATON이 사용자와 작업공간·시즌 권한을 확인한 뒤 PRD-0027 요약과
PRD-0028 필터 목록을 중계하고 화면에 사용하는 것이다. BRIEF의 빈 요약을 권한 판정으로
사용하지 않고, 요약과 목록 사이의 요청 시점 차이를 보존하며 필터 변경 시 커서를 초기화한다.
