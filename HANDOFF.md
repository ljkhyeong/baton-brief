...
- 현재 관심 항목 단건·상태별 목록·활성 요약·상태 전이 증거와 원자적 전체 재구축
...
같은 최종 코드로 BRIEF·서비스 Caddy 이미지를 빌드하고 기존 CI의 Caddy 설정·HTTPS 조립
...
BATON의 요약 중계 API·client·화면과 새 경로의 교차 서비스 검증은 이번 작업에 포함하지
않았다. 기존 조회·생성 교차 검증을 요약 경로의 완료 근거로 확대하지 않는다.
...
- BATON의 현재 활성 관심 항목 요약 중계 API·client·화면과 교차 서비스 검증
...
요약을 제외한 기존 사용자 조회·생성 연결의 로컬 구현과 교차 서비스 검증은 완료했다.
...
다음 제품 연결 작업은 BATON이 사용자와 작업공간·시즌 권한을 확인한 뒤 PRD-0027 요약을
중계하고 화면에 사용하는 것이다. BRIEF의 빈 요약을 권한 판정으로 사용하지 않고 요약과
목록 사이의 요청 시점 차이를 보존한다.
