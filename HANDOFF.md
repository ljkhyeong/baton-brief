...
| BRIEF `7d42310` 조회·업무 종류 필터·주간 이력, 2026-09-12 | `./gradlew test :bootstrap:bootJar` 성공(27초). `bootstrap` 38건 통과, 변경 없는 도메인 6건은 기존 결과 재사용. JDK 21.0.10·PostgreSQL 18.6. 로그 `/tmp/brief-weekly-history-20260912.log` | 기존 조회·생성·수신·인증·업무 종류 필터와 주간 이력의 주차·시간대·범위 격리, 필터 적용 후 페이지 조회, 새 생성 후 기존 커서, 날짜·시간대 오류·연도 경계 확인. 실패·제외 없음. 이후 문서만 변경. BATON 중계·화면 연결과 원격 배포는 미실행. 계약 ZIP 입력은 변경 없음 |
