...
구현·검증했다. BATON `f81ee47`의 선택 실행 테스트는 실제 BATON·BRIEF 실행 JAR과
MySQL 8.4·PostgreSQL 18.4를 함께 기동해 V23 outbox부터 BRIEF 수신·현재 투영까지 연결했다.
역순 리비전의 선행 차단, BRIEF 미기동 네트워크 실패의 재시도, 최초 `202`, 응답 유실
상태를 재현한 동일 이벤트 `200`, 심각도 변경과 `RESOLVED` 투영을 확인했다. 원본 변경·
초기 정합화부터 outbox
생성까지 한 교차 서비스 시나리오로 검증한 것은 아니며 운영 인증·HTTPS도 포함하지 않는다.
...
- PRD-0019에서 v1 세 타입과 fingerprint를 보존하고 v2 다섯 타입·원본 심각도·V7 저장과
  ...
  HTTP 송신기도 구현·검증했다. BATON `f81ee47`은 실제 두 실행 JAR과 두 데이터베이스에서
  outbox 이후의 장애 재시도·동일 재전달·순서·심각도·해소 수렴을 검증했다. 다음 진입점은
  권위 있는 원본 변경·초기 정합화부터 시작하는 단일 흐름과 운영 인증·HTTPS 검증이다.
...
- WATCH, RELAY, GO 생산자와의 종단 간 연동은 없다. BATON은 로컬 선택 실행 테스트에서
  실제 두 프로세스의 V23 outbox→HTTP→BRIEF PostgreSQL 수렴을 검증했다. 다만 권위 있는
  원본 변경·초기 정합화부터 outbox 생성까지 한 흐름으로 다시 증명하지 않았고, 운영
  인증·HTTPS·스테이징 활성화도 남아 있다.
