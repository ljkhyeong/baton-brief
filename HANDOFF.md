...
| BRIEF `b1dc856` 조회·업무 종류 필터, 2026-09-12 | `./gradlew test :bootstrap:bootJar` 성공(19초). `bootstrap` 37건 통과, 변경 없는 도메인 6건은 기존 결과 재사용. JDK 21.0.10·PostgreSQL 18.6 | 기존 조회·생성·수신·인증과 새 `eventType` 필터의 v1·v2 종류, 필터 조합·커서·범위 격리·해소·재구축 결과 확인. 실패·제외 없음. BATON 중계·화면 연결과 원격 배포는 미실행. 계약 ZIP 입력은 변경 없음 |
