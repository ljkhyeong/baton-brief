...
| --- | --- | --- |
| BRIEF `07bb1f0` 중복 JSON 필드 거부, 2026-09-12 | `./gradlew test :bootstrap:bootJar contractsZip` 성공(29초), bootstrap 40건 통과·ArchUnit 4건 및 도메인 6건 성공 결과 재사용. 중복 eventId 요청의 기존 `202` 저장을 재현한 뒤 eventId·state·weekStart 중복의 `400 ProblemDetail`과 저장 0건 확인. 로그 `/tmp/brief-duplicate-json-before-20260912.log`·`/tmp/brief-duplicate-json-after-20260912.log`·`/tmp/brief-duplicate-json-tests-20260912.log` | MockMvc·PostgreSQL 18.6·JDK 21.0.10·Jackson 3.1.5. 정상 수신·재전달·생성·조회·재구축 및 초기화 검사 포함. Spring Boot 4의 `spring.jackson.read.strict-duplicate-detection`을 사용하며 별도 본문 검사기 없음. 전체 diff·구조·문서 링크 확인, ZIP 문서 5개·내부 링크 8개와 JAR 설정 반영 확인. 중복 필드를 보내던 요청은 이제 거부한다. 이벤트 스키마·계약 버전 유지. 원격 생산자·실제 HTTP 서버·배포 검증은 미실행 |
| BRIEF `5cbc247` 운영 명령 초기화 검사, 2026-09-12 | `./gradlew test :bootstrap:bootJar` 성공(19초), bootstrap 39건 통과·ArchUnit 4건 및 도메인 6건 성공 결과 재사용. 초기 검사 등록과 빈 생성 전 실패 확인. 실제 JAR로 잘못된 옵션 4건 차단·정상 RECEIPT/ANOMALIES/REBUILD 3건 확인. 로그 `/tmp/brief-operations-startup-tests-20260912.log`·`/tmp/brief-operations-startup-runtime-20260912.log` | JDK 21.0.10·PostgreSQL 18.6. 변경 전 JAR는 거부 전에 V1~V10을 실행했으나, 변경 후에는 테이블 0개 유지·Hikari/Flyway/Tomcat 미기동 확인. 정상 명령의 JSON/로그 분리·수신 기록 보존·마이그레이션 미실행 확인 후 임시 DB 정리. 기존 주간 집계 개선을 포함한 최신 JAR 생성 완료. 등록 파일은 JAR 루트 `META-INF/spring.factories`에 포함되며 `BOOT-INF/classes/META-INF`가 아님. 전체 diff·구조·문서 링크 확인. 원격 CI·배포·계약 ZIP은 미실행 |
