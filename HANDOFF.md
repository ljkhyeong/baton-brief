...
| BRIEF `7d54ba6` 운영 경보, 2026-09-12 | Prometheus 3.14.0의 `promtool check config`·`test rules`로 규칙 4개·시나리오 10개 통과. `42b744f` JAR·JDK 21.0.10·PostgreSQL 18.6에서 실제 연결 대기 지표 `0 → 1 → 0`과 잠금 해제 후 요청 11건의 `202` 확인. 로그 `/tmp/brief-db-wait-rules-20260912.log`·`/tmp/brief-db-wait-runtime-20260912-retry.log` | 짧은·간헐적 대기, 다른 서비스, 대기 없는 최대 연결 사용은 경보에서 제외하고 지속 대기·복구 확인. 첫 검증은 임시 풀 1개로 Flyway 초기화 중 연결 부족이 발생해 기본 10개로 재검증했다. 임시 앱·DB 정리 완료. 앱 소스·의존성·실행 구성은 같아 아래 Gradle 결과 재사용. 원격 배포·외부 알림은 미실행 |
| BRIEF `f5c26fb` 주간 해소 필터, 2026-09-12 | `./gradlew test :bootstrap:bootJar` 성공(17초). bootstrap 39건 통과, 도메인 6건 기존 결과 재사용. JDK 21.0.10·PostgreSQL 18.6. 로그 `/tmp/brief-resolution-filter-20260912.log` | 업무 종류별 건수·목록, v1·v2, 필터 생략·빈 결과·잘못된 값, 범위 격리·커서·재활성화·재구축·서비스 인증 확인. 기존 주간 경계·비교 조건부 조회를 포함한 전체 테스트 통과. 실패·제외 없음. 실행 JAR도 이 커밋 기준으로 갱신. BATON 필터 중계·화면 연결·원격 배포는 미실행, 계약 ZIP 입력은 변경 없음 |
