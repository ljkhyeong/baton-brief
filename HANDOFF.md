...
추가 이용료 없는 Prometheus와 PostgreSQL 백업·격리 복원 확인을 제공한다. 운영 알림은 Slack으로
정했으며 공용 Alertmanager 연결 대상과 실제 웹훅은 미등록이다. [외부 연동 검토](docs/operations/external-integrations.md)를 따른다.
...
| BRIEF `0d9b064` Slack 경보 연동, 2026-09-12 | Prometheus 3.14.0 설정·경보 13개 시나리오, Alertmanager 0.32.1 설정·BRIEF/타 서비스 분기, actionlint 1.7.12 통과. 실제 두 수집 도구와 Python 수신 대역으로 빈 대상의 미발송·파일 변경 반영·HTTPS 전달·Slack 경보/해제·내부 URL 비노출 확인. 로그 `/tmp/brief-slack-config-check-20260912.log`·`/tmp/brief-slack-runtime-20260912-retry.log` | 검증 컨테이너는 비루트·읽기 전용·외부 네트워크 및 호스트 포트 없음. 시험에만 판정·발송 간격 단축, 임시 CA·웹훅 사용 후 정리. 최초 시험 도구의 초기 빈 목록 처리 오류를 수정한 뒤 통과. 기존 CI 미사용 반복 변수 경고도 정리. 앱·빌드·계약 입력은 `6ed7922`와 같아 Gradle·JAR 재검증 제외. 실제 Slack·공용 Alertmanager·원격 CI·배포는 미연결·미실행 |
...
1. Slack 웹훅과 공용 Alertmanager의 비공개 HTTPS 주소가 준비되면 [연결 절차](docs/operations/external-integrations.md)를 적용한다.
