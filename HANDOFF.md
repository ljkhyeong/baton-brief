...
받는다. UTC의 `0000-01-03`·`9999-12-27` 생성과 저장된 주간 조회, 기존에 `500`을 만들던
범위 밖 날짜와 `+10000` 연도의 두 경로 `400 ProblemDetail`을 PostgreSQL 통합 테스트로
확인했다. 현재 시점 기준 제한이나 별도 검증·예외 처리 클래스는 추가하지 않았다.
...
포함해 계약 ZIP을 다시 생성했다. 새 실행 JAR 생성까지 확인했으며 V8 스테이징 이미지와
BATON 교차 서비스 시나리오는 이번 작업에서 실행하지 않았다.
