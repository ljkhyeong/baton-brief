...
구현·검증했다. 두 서비스를 함께 기동한 실제 BRIEF HTTP 종단 간 전달은 아직 없다.
...
- PRD-0019에서 v1 세 타입과 fingerprint를 보존하고 v2 다섯 타입·원본 심각도·V7 저장과
  ...
  HTTP 송신기도 구현·검증했다. 다음 교차 저장소 진입점은 실제 BRIEF 종단 간 검증이다.
...
- BATON, WATCH, RELAY, GO 생산자와의 종단 간 연동은 없다. BRIEF 이벤트 v2 소비는
  ...
  재조정, 원본 변경·자동 회차 생성의 같은 트랜잭션 연결, outbox 송신기와 HTTP 결과
  분류까지 검증했지만 두 프로세스 사이 종단 간 전달은 아직 없다.
