...
JSON Schema와 예시로 제공한다. BRIEF 소비자 구현과 예시의 호환성은 검증하지만, BATON
실제 serializer·outbox·송신기나 종단 간 전달이 완료됐다는 뜻은 아니다.
...
현재 버전 `2.0.0-rc.1`은 BRIEF 소비자와의 호환성만 검증한 사전 버전이다. BATON 저장소가
이 계약 팩을 고정하고 실제 serializer 출력과 전달 경계를 검증하기 전에는 안정 버전으로
올리지 않는다.
