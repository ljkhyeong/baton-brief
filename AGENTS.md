...
- 현재 활성 관심 항목 목록은 `eventType`·`sourceReference` 복합 정체성 오름차순과 배타
  키셋을 사용한다. `RESOLVED`를 제외하고 커서를 스냅샷으로 해석하지 않으며, 심각도
  우선의 불변 에디션 선정 순서를 복제하지 않는다.
...
- PRD-0002의 이벤트·투영·에디션 경로, PRD-0003의 에디션 이력 경로, PRD-0004의 표준
  ...
  PRD-0013의 현재 관심 항목 단건 조회, PRD-0014의 현재 활성 관심 항목 키셋 조회와
  인증 없는 로컬 MVP 경계를 따른다.
