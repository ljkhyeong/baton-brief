...
- 스테이징 Caddy HTTPS 앞단은 정확한 `POST /api/v1/events`만 BRIEF로 전달하고 다른 모든
  ...
  외부에 노출하지 않는다.
- BATON 백엔드의 조회·에디션 생성은 이벤트 수신 Bearer와 분리한 서비스 인증과 비공개
  네트워크 경로만 사용한다. 공개 Caddy 허용 목록에는 이 경로를 추가하지 않는다.
...
- ADR-0002를 기술 스택의 기준 문서로 사용한다. Kotlin/JVM·Kotlin BOM 2.4.10, Java 21
...
- `docs/README.md`에 색인된 채택 PRD를 제품 동작의 기준으로 사용한다. PRD-0008의 직접
  동시성 검증 범위는 재구축과 지원 이벤트 수신이며, 에디션 생성·다른
...
- 선택적인 `https` profile은 digest로 고정한 Caddy와 자동 HTTPS를 사용한다. 실제
  ...
  확대하지 않는다.
- BATON 주도 에디션 생성의 현재 구현·검증 범위는 `HANDOFF.md`를 따른다. 로컬 교차 서비스
  검증을 실제 스테이징 인증서·비밀·네트워크와 장애 주입 검증으로 확대하지 않는다.
  브로커와 그 밖의 외부 시스템 연동 어댑터도 결정된 것으로 가정하지 않는다.
