...
- 두 서비스를 동시에 중단하지 않고 환경 변수 변경과 순차 배포로 token을 바꿀 수 있다.
- 사용자 세션이나 새 권한 모델을 만들지 않고 생산자 경계만 보호한다.

### 단점

- 하나의 공유 비밀이므로 생산자별 권한 분리와 자동 회전 기능이 없다.
- 직전 token을 제거할 때까지 기존 비밀의 유효 시간이 일시적으로 늘어난다.
- 로컬 Caddy HTTPS 검증만으로 실제 공인 인증서·BATON 원격 전달과 비밀 관리 절차가
  준비되었다고 주장할 수 없다.
