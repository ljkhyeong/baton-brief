...
현재 브로커나 외부 시스템 연동 어댑터를 채택하지 않는다. ADR-0004와 PRD-0021은
loopback HTTP 앞단까지의 최소 스테이징 컨테이너 조립만 채택한다. 공개 DNS, TLS 종단,
reverse proxy, 방화벽과 실제 운영 배포 방식은 결정하지 않는다. 로컬 MVP의 내부 HTTP
엔드포인트는 PRD-0002를 따르고 BATON 이벤트 수신 전용 Bearer는 ADR-0003과 PRD-0020을
따른다. 다른 API의 운영 인증·인가는 별도 결정 전까지 만들지 않는다.
...
ADR-0004는 스테이징 컨테이너에 한해 특정 JDK 공급자와 실행 이미지를 후속 채택했다.
이를 공개 HTTPS, 인증서, 운영 데이터베이스와 장기 배포 방식을 결정한 것으로 확대하지
않는다.
