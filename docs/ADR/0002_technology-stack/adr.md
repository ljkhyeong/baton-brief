...
비용이 낮다. BRIEF가 다룰 브리프, 버전이 있는 이벤트와 제한된 상태는 Kotlin의
...
스테이징 컨테이너 구성을, ADR-0005와 PRD-0022는 선택적인 Caddy 이벤트 수신 HTTPS 앞단을
...
- SQL과 마이그레이션을 명시적으로 관리해 투영, 수신함과 브리프 저장 동작을 드러낼 수
  있다.
- 브리프, 버전이 있는 이벤트와 제한된 상태를 Kotlin 타입으로 명시적으로 표현할
  수 있다.
