...
한다. 다만 공개 DNS, 인증서와 reverse proxy 제품은 아직 정해지지 않았다.
...
- 실행 컨테이너는 UID/GID `10001`, 읽기 전용 루트 파일시스템, `/tmp` tmpfs와 모든 Linux
  capability 제거를 사용한다.
- `compose.staging.yml`은 BRIEF와 PostgreSQL 18.4만 조립한다. PostgreSQL은 내부
  데이터 네트워크에서만 실행하고 호스트 포트를 공개하지 않는다.
- BRIEF HTTP 포트는 기본적으로 `127.0.0.1`에만 바인딩한다. 공개 HTTPS, 인증서와
  reverse proxy는 배포 환경이 소유하고 애플리케이션 이미지에 TLS 종단을 넣지 않는다.
...
- 공개 TLS 제품을 정하기 전에 애플리케이션 실행 경계를 독립적으로 검증할 수 있다.
...
- loopback 바인딩만으로는 다른 호스트의 BATON이 접근할 수 없다. 실제 전달에는 별도
  reverse proxy와 공개 HTTPS 환경이 필요하다.
