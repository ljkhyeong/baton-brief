...
- BRIEF HTTP 포트는 기본적으로 `127.0.0.1`에만 바인딩한다. 후속 HTTPS 앞단과 인증서는
  배포 환경이 소유하고 애플리케이션 이미지에 TLS 종단을 넣지 않는다.
- 후속 ADR-0005의 선택적인 Caddy profile도 BRIEF의 loopback HTTP 실행 책임을 바꾸지
  않는다.
