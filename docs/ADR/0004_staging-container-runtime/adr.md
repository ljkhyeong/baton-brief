...
유일한 호스트 인입 경계로 유지하도록 직접 게시 계약을 제거한다.
...
- BuildKit이 해석하는 Dockerfile frontend도 `docker/dockerfile:1.7`의 다중 플랫폼 digest로
  고정한다.
- 빌드와 실행 이미지는 Eclipse Temurin 21.0.12+8 Alpine 이미지를 digest로 고정한다.
...
- BRIEF 프로세스는 컨테이너 내부 네트워크 요청을 받도록 `0.0.0.0:8080`에서 실행하되,
  ...
  사용한다. HTTPS 앞단과 인증서는 배포 환경이 소유하고 애플리케이션 이미지에 TLS 종단을
  넣지 않는다.
...
- 컨테이너 내부 healthcheck로 HTTPS 앞단과 독립적인 애플리케이션 실행 경계를 검증할 수
  있다.
- BRIEF 직접 게시가 없어 공개·서비스 Caddy의 허용 목록을 우회하는 호스트 경로가 없다.
