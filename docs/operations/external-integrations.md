...
아래 입력이 준비되거나 요구사항이 달라질 때 해당 항목만 다시 검토한다.

| 항목 | 필요한 입력 | 다음 작업 |
| --- | --- | --- |
| Slack·Discord 운영 알림 | 사용할 채널의 웹훅, 공용 Alertmanager의 비공개 HTTPS 주소·인증 정보 | 기존 설정에 연결하고 실제 경보·해제 수신 확인 |
| BRIEF 외부 백업 | 이미 보유한 별도 저장소의 종류·용량·접속 방법 | 기존 덤프를 rclone으로 암호화 전송하고 다운로드·복원 확인 |
| Cloudflare DNS 자동 갱신 | 공인 IP의 고정·변동 여부 | 변동 IP일 때만 ddclient 연결 검토 |
| 공용 앞단의 인증서 갱신 | 준비된 인증서의 종류·적용 대상·현재 갱신 방식 | 기존 갱신 기능과 중복되지 않게 연결 |

CAL의 직접 캘린더 API 연동은 현재 `.ics` 구독보다 빠른 갱신이나 양방향 편집이 필요해질 때
재검토한다. 홈서버 설치·배포는 별도 요청 범위이며 위 연결 조건 확인을 대신하지 않는다.

## Slack·Discord 운영 알림

```text
BRIEF 지표 → 기존 Prometheus 경보 → 공용 Alertmanager → Slack·Discord 운영 채널
```

앱 코드·DB·공개 API는 바꾸지 않는다. Alertmanager의 기본 채널 연동이 발송·재시도·경보 묶음과
해제 알림을 처리한다. BRIEF 본문이나 업무 이벤트는 이 경로로 보내지 않는다.

필요한 채널만 선택한다. 웹훅 파일 경로는 Alertmanager 컨테이너 안의 경로다.
...
1. Slack은 기존 앱의 [Incoming Webhooks](https://docs.slack.dev/messaging/sending-messages-using-incoming-webhooks/)에서,
   ...
   Discord 포럼·미디어 채널은 이 예시의 대상이 아니다. URL을 저장소 밖 비밀 파일에 저장하고
...
4. 연결은 HTTPS이며 인증서의 호스트명을 검증한다. 사설 CA를 쓰면 `prometheus.yml`의 해당
   ...
   인증서 검증을 끄거나 관리 포트를 공개하지 않는다. Alertmanager의 채널 발송 경로만 외부 HTTPS가 필요하다.
5. 실행 버전의 `promtool check config`와 `amtool check-config`로 설정을 확인하고 적용한다.
   ...
   승인된 시험 경보로 선택한 채널의 도착과 해제 알림을 확인한 뒤 사용한다.
...
Prometheus는 경보에 `service=brief`를 붙인다. 채널 수신 경로는 이 값만 선택하며 다른 서비스 경보는
...
- [Alertmanager Discord 기본 연동](https://prometheus.io/docs/alerting/latest/configuration/#discord_config)
