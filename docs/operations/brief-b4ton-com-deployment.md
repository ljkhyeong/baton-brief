...
Cloudflare DNS와 Ubuntu 홈서버, 공인 IP·인증서, 공유기 80·443 포트포워딩은 준비됐고 k3s는 미구축이다.
공개 주소는 `b4ton.com`, 각 서비스는 `<서비스명>.b4ton.com`을 사용한다.
아래는 기존 Compose 배포 참고 절차이며 k3s 설치 절차가 아니다.
실제 배포 시 Cloudflare에서 다음 레코드를 확인한다. 대상 주소는 실제 공인 IP를 사용한다.
...
통합하고, BRIEF 이벤트 허용 경로·헤더 제거·네트워크 격리를 같은 구성에서 검증해야 한다.
...
기존 Compose의 Caddy 자동 HTTPS는 공인 인증서 발급·갱신을 담당한다. 향후 공용 앞단에서는
준비된 인증서의 종류·갱신 방식에 맞춘다. DNS 등록만으로 앱 배포나 인증서 적용이 완료된 것은 아니다.

## 구축 순서

1. 서버 위치·접속 방법·실제 IP와 공개 프록시 운영 담당자를 정한다. 같은 서버 배치는 공개
...
3. BRIEF와 연동 코드를 적용한 BATON을 내부에서 기동하고 애플리케이션·DB 상태를 확인한다.
...
5. BATON 실제 원본 변경을 전달해 최초 `202`, 동일 이벤트 재전달 `200`, 심각도 변경과
   해소 상태가 BRIEF에 반영됐는지 확인한다. 합성 이벤트를 실제 업무 작업공간에 임의로 넣지 않는다.
