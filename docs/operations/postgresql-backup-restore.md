...
CI에서는 이벤트를 저장한 검증용 DB로 백업 생성·격리 복원과 손상 파일의 실패 처리를 확인한다.
실제 운영 백업의 복원 가능 여부는 해당 파일로 별도 확인해야 한다.
...
Docker가 실행 중인 환경에서 백업 파일의 실제 절대 경로를 지정한다.

```bash
BRIEF_BACKUP_FILE=/absolute/backup-directory/brief-.../database.dump \
  bash ops/verify-postgresql-backup.sh
```

스크립트는 Compose에 고정된 PostgreSQL 이미지로 임시 DB를 만들고 실제 복원을 실행한다.
별도 클라이언트 설치나 `.env.staging`·운영 비밀은 필요 없다. 이미지가 로컬에 없으면 처음
실행할 때 내려받으며, 복원할 DB를 담을 Docker 디스크 공간이 필요하다.

컨테이너는 `postgres` 사용자로 실행하고 네트워크·호스트 포트·원본 DB 볼륨을 연결하지 않는다.
백업은 표준 입력으로 전달하므로 파일 권한 `0600`을 유지할 수 있다. 복원 성공·실패 후
컨테이너와 임시 DB는 자동 삭제하며 원본 백업은 그대로 둔다. 실패하면 0이 아닌 종료 코드를 반환한다.

이 명령은 파일 손상이나 SQL 실행 오류로 복원할 수 없는 백업을 찾는다. 백업 전후의 데이터
일치나 애플리케이션 동작까지 확인하지는 않는다. 복원한 DB를 유지하며 아래 항목을 확인하려면
수동 복원 절차를 따른다.
