...
작업공간·시즌 범위에서 탐색할 수 있게 한다. 저장된 이상 수신 기록을 조회하며
...
`ingestionSequence`는 바뀌지 않지만 충돌 기록은 최초 수신 뒤에 추가될 수 있다. 그 결과
...
탐색에는 나타나지 않을 수 있다. 호출자는 최신 이상 수신 기록을 다시 확인할 때 커서를 버리고
첫 페이지부터 새로 조회한다.
