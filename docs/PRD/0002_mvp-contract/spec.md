...
- 정렬·선택된 고정 항목 상태를 정규화해 `stateFingerprint`를 계산한다. 같은
  작업공간, 시즌, `weekStart`, `zoneId`, 규칙 버전과 `stateFingerprint`에 대한 반복 생성
  요청은 하나의 논리적 에디션만 만든다. 생성 시각이나 원본 커서만 달라졌다는
  이유로 새 에디션을 만들지 않는다.
...
- 같은 `stateFingerprint`의 반복 요청이 에디션을 중복 생성하지 않고, 선택 상태가 달라지면
  세대가 증가한다.
