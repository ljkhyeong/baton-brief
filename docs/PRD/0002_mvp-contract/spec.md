...
첫 MVP의 구현 범위를 정한다. BRIEF는 BATON의 업무 변경 이벤트를 내부 HTTP로 받아
멱등하게 기록하고, 사유를 포함한 점검 항목에 반영한 뒤 작업공간·시즌 단위의 주간
브리프를 생성·조회한다.
...
않으며, 계약에 없는 JSON 필드는 무시하지 않는다. 같은 객체에 필드명을 반복하면 값이 같아도
...
- 빈 문자열과 JDK 21 `Character.isWhitespace` 기준으로 공백만 있는 문자열은 거부한다.
  기존 수신 검증과 같은 기준이며 NBSP(`U+00A0`) 같은 비분리 공백은 원문 그대로 허용한다.
- `U+0000`과 짝이 없는 UTF-16 surrogate는 거부한다. 정상 surrogate 쌍으로 표현되는
  이모지 등 보충 문자는 허용한다.
...
- 리비전이 하나 이상 건너뛴 이벤트는 도착 순서를 원본 사실의 판단 기준으로 삼지 않는다. 이벤트
  자체는 `APPLIED_WITH_GAP`으로 적용하고 점검 항목에 리비전 공백을 기록한다.
...
- AI 생성 결과, 수신 기록 도착 시간 또는 BRIEF의 추론으로 이유·심각도·상태를 바꾸지
  않는다.
- 표시 가능한 항목은 작업공간·시즌, 이유 코드, 심각도, 원본 참조값,
...
이벤트별 최초 충돌 기록은 대체 계약 전까지 보존하되, 재구축은 `UNSUPPORTED`를 제외한
...
- 생성 트랜잭션에서 해당 작업공간·시즌에 보존된 지원 수신 기록의 최대
  `ingestion_sequence`를 `sourceCursor`로 저장한다. 해당 기록이 없으면 `0`이다.
  이 값만으로 원본 이벤트가 모두 전달됐다고 판단할 수 없으며 수신 시각을 나타내지도 않는다.
...
- 전역 최신 조회가 작업공간·시즌에서 생성 번호가 가장 큰 브리프를 반환하고 단건 조회가 동일 고정 스냅샷을
  반환한다.
- PRD-0009의 주간 최신 조회가 정확한 작업공간·시즌·주간·시간대 범위에서 생성 번호가 가장 큰 브리프를
