...
PRD-0013은 정체성을 이미 아는 호출자가 현재 관심 항목 한 건을 확인하게 한다. BATON UI가
현재 활성 항목을 발견하려면 모든 이벤트 종류와 원본 참조를 미리 알고 있어야 한다.
...
- 다음 페이지 존재 여부는 `limit`보다 한 건 더 읽어 판단하며 전체 개수 조회는 하지
  않는다.
- `RESOLVED` 항목은 목록에서 제외하지만 PRD-0013의 정확한 단건 조회로 현재 해소 상태를
  확인할 수 있다.
...
- `RESOLVED` 항목은 후속 첫 페이지에서 제외되고 PRD-0013 단건 조회에는 남는다.
