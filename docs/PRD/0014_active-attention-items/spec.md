...
- 커서는 `(eventType, sourceReference)` 복합 식별자의 배타적 재개 위치다.
...
이 정렬은 현재 항목을 빠짐없이 탐색하기 위한 정체성 순서다. 심각도 우선의 브리프
