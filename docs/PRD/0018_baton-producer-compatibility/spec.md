...
- 상태: 채택됨
- 결정일: 2026-08-22
- 구현 상태: BATON 생산 의미·BRIEF 소비자 v2·RC 계약 팩·생산자 직렬화·신호 스트림·불변 outbox·설정형 시간 재조정·원본 변경 자동 연결 구현, 송신·종단 간 검증 미구현
...
원본 변경과 자동 회차 생성도 같은 트랜잭션 재조정에 연결했다. outbox 송신기와 종단 간
전달은 아직 구현하지 않았다.
...
재조정에 연결하고 원본·outbox 원자적 롤백을 검증했다. 전체 빌드도 성공했다. 현재 다음
구현 진입점은 커밋 뒤 outbox 전달 생명주기다. outbox 송신기와 BATON→BRIEF 종단 간
전달을 검증하기 전에는 생산자 연동 완료를 주장하지 않는다.
