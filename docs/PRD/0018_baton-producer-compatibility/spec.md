...
- 상태: 채택됨
- 결정일: 2026-08-22
- 구현 상태: BATON 생산 의미·BRIEF 소비자 v2·RC 계약 팩 구현, BATON 생산자 미구현
...
이 결정 뒤 PRD-0019에서 BRIEF 이벤트 v2 소비를 구현했다. 이는 BATON 실제 직렬화,
outbox·송신기나 생산자 연동 완료를 뜻하지 않는다.
...
다음 구현 진입점은 BATON 저장소가 BRIEF `2.0.0-rc.1` 계약 팩을 고정하고 실제 serializer
출력을 검증한 뒤, 생산자 신호 스트림·전용 outbox와 송신기를 구현하는 작업이다. BRIEF
계약 예시만으로 생산자 호환이나 종단 간 전달 성공을 주장하지 않는다.
