...
| 그 밖의 32비트 양수 | 알려진 열거형만 JSON으로 해석 | `UNSUPPORTED`로 최초 수신 기록을 보존하고 투영하지 않음 |
...
파생한다. `ACTIVE`·`RESOLVED`, 복합 식별자, 리비전 공백, 오래된 이벤트, 현재 단건·목록·
전이 조회와 브리프 고정 규칙은 기존 계약을 그대로 사용한다.
...
- 수신 기록, 점검 항목과 브리프 항목의 이벤트 종류 제약은 v1·v2 여덟 타입을
  허용한다.
...
- 현재 투영과 브리프의 `severity` 저장 값은 계속 `HIGH`·`MEDIUM`만 사용한다.
...
- 대표 기존 행에 마이그레이션을 적용했을 때 새 열은 `null`이고 기존 현재 투영·브리프
  항목을 보존한다.
