...
- 상태: 채택됨
- 결정일: 2026-08-18
- 범위: 저장된 두 브리프의 결정적 항목 차이 조회

## 목적

PRD-0002와 PRD-0003은 브리프의 단건·이력 조회를 제공하지만, BATON UI가 두 시점
사이에서 어떤 항목이 추가·제거·변경되었는지는 직접 계산해야 한다.

저장된 두 브리프 스냅샷을 안정적인 항목 키로 비교하는 읽기 전용 API를 추가한다. 현재
투영이나 원본 상태를 다시 조회하지 않으며, 저장된 브리프와 현재 투영을 변경하지 않는다.
...
| 메서드 | 경로 | 의미 |
|---|---|---|
| `GET` | `/api/v1/editions/{targetEditionId}/changes?fromEditionId={baseEditionId}` | 기준 브리프에서 대상 브리프로의 항목 차이 조회 |

- `targetEditionId`는 비교 결과의 대상인 `to` 브리프 식별자다.
- 필수 질의 매개변수 `fromEditionId`는 비교 기준인 `from` 브리프 식별자다.
- 두 식별자는 UUID다. 잘못된 UUID나 누락된 `fromEditionId`는 `400 Bad Request`다.

## 비교 범위

- 두 브리프는 같은 `workspaceId`와 `seasonId`에 속해야 한다. 둘 중 하나라도 다르면
...
- 같은 작업공간·시즌이면 서로 다른 `weekStart`, `zoneId`, `ruleVersion`의 브리프도
  비교할 수 있다.
- 생성 순서와 관계없이 비교할 수 있다. URL의 `targetEditionId`가 비교 대상(`to`),
  질의 매개변수 `fromEditionId`가 비교 기준(`from`)을 정한다.

사용자 권한은 BATON이 판정하고, 서비스 인증과 접근 제한은
[PRD-0025](../0025_baton-service-api-security/spec.md)를 따른다.

## 성공 응답

성공 응답은 `200 OK`이며 다음 필드를 가진다.
같은 비교 결과를 이미 받은 경우 [PRD-0012](../0012_edition-etag/spec.md)에 따라 응답의 ETag를
`If-None-Match`로 보내면 본문 없는 `304 Not Modified`를 받을 수 있다.
...
`added`와 `changed`는 대상 브리프에 저장된 항목 순서를 유지한다. `removed`는 기준
브리프에 저장된 항목 순서를 유지한다. 따라서 별도 정렬 선택지 없이도 PRD-0002의
저장 순서가 재현된다. PRD-0029 적용 뒤 새 브리프는 그룹 순서 안에서 `severity` 내림차순,
`reasonCode`, `sourceReference` 오름차순을 사용한다.

`removed`는 비교 대상 브리프에 해당 항목이 없다는 뜻이다.
해당 항목의 현재 상태가 `RESOLVED`라는 의미는 아니다.

## 불변성과 호환성

- 비교는 저장된 두 불변 스냅샷만 읽으며 현재 `AttentionItem` 투영을 조회하거나 다시
  조합하지 않는다.
- 비교 요청은 브리프, 항목, 수신 기록, 투영과 원본 커서를 생성·수정·삭제하지 않는다.
...
- 현재 투영과 과거 브리프 비교
- 세 개 이상 브리프의 추세·집계 비교
- 비교 결과 페이지네이션, 검색과 정렬 선택지
- BATON 멤버십, 인증·인가, 외부 공개와 운영 배포

## 수용 기준

- 같은 작업공간·시즌의 두 브리프를 비교해 안정적인 키 기준의 `added`, `removed`,
...
- 같은 비교 키에서 `aggregateRevision` 또는 `revisionGap`만 달라도 `changed`에
  `before`·`after`를 반환한다.
- 같은 작업공간·시즌이면 주간·시간대·규칙 버전이 달라도 생성 순서와 관계없이 비교할 수 있다.
- 어느 식별자든 없으면 `404`, 작업공간이나 시즌이 다르면 `400`이며 모두
  `application/problem+json`으로 응답한다.
- 비교 전후에 두 브리프와 현재 투영의 저장 상태가 바뀌지 않는다.
