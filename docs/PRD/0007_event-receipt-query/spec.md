...
`SourceEventReceipt`를 이벤트 식별자로 조회해 최초 수신 결과와 제한된 충돌 기록을 확인할
수 있게 한다.

이 조회는 전달 시도 이력, 원본 사실의 최신 상태나 생산자 전달 완전성을 판정하지 않는다.
최초 수신 기록을 읽을 뿐이며 기존 멱등 수신, 투영과 브리프 의미를 바꾸지 않는다.
`eventId`를 모를 때 작업공간·시즌별 이상 수신 기록을 찾는 조회는 PRD-0011에서 정의한다.
...
| 메서드 | 경로 | 의미 |
|---|---|---|
| `GET` | `/api/v1/events/{eventId}/receipt` | 이벤트 식별자의 최초 수신 기록 조회 |
...
- PRD-0019의 v2 수신 뒤에도 같은 최초 기록 표현을 사용하며 `sourceSeverity`만 nullable
  안전 필드로 포함한다.
- PRD-0011의 이상 수신 기록 조회는 이 단건 응답과 같은 안전한 필드를 반환하지만,
...
- 보존 기간·용량, 삭제·압축 구현, 격리 해제와 재처리 정책
