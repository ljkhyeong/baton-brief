...
PRD-0014는 이 단건 계약을 바꾸지 않고 현재 `ACTIVE` 항목만 복합 정체성 키셋으로
발견하는 별도 목록 경로를 추가한다. `RESOLVED` 현재 상태는 계속 이 단건 조회로 확인한다.
...
- `ACTIVE` 외 상태의 목록, 자유 검색, 정렬 선택, 전체 개수와 offset 페이지네이션
- 과거 투영 상태 이력과 변경 이력
